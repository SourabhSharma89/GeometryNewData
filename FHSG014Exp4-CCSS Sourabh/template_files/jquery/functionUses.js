...
$(document).ready(function(){
	...
	Revert_Minus_sign = function(str) {
	 ...
	 ConvertSmalltoBig = (num)=>{
	 var FinalStr = "";
	 if(num<0){
		 FinalStr = "– "+Math.abs(num);
	 }else{
		 FinalStr = num;
	 }
	 return FinalStr;
...
if (((eval_ans[eval_c] == "*") || (eval_ans[eval_c] == "^") || (eval_ans[eval_c] == "+") || (eval_ans[eval_c]=="-")|| ((eval_ans[eval_c] == "/") && (eval_ans[eval_c-1] != undefined)) ) && (((isNaN(eval_ans[eval_c-1]) == true) && (eval_ans[eval_c-1])!=")") || (eval_ans[eval_c-1] == "("))) {
	...
	xCordinateFun = (val1S)=>{
	 ...
	 if(Number(val1S) == -8){
		...
		else if(Number(val1S) == -7){
			num11 = 75;
		}
		else if(Number(val1S) == -6){
			num11 = 100;
		}
		else if(Number(val1S) == -5){
			num11 = 125;
		}
		else if(Number(val1S) == -4){
			num11 = 150;
		}
		else if(Number(val1S) == -3){
			num11 = 175;
		}
		else if(Number(val1S) == -2){
			num11 = 200;
		}
		else if(Number(val1S) == -1){
			num11 = 225;
		}
		else if(Number(val1S) == 0){
			num11 = 250;
		}
		else if(Number(val1S) == 1){
			num11 = 275;
 ...
 yCordinateFun = (val2S)=>{
	 //alert(Number(val2S) == -3)
	 var num21 = 0;
	 if(Number(val2S) == -8){
			num21 = 450;
		}
		else if(Number(val2S) == -7){
			num21 = 425;
		}
		else if(Number(val2S) == -6){
			num21 = 400;
		}
		else if(Number(val2S) == -5){
			
			num21 = 375;
		}
		else if(Number(val2S) == -4){
			num21 = 350;
		}
		else if(Number(val2S) == -3){
			num21 = 325;
		}
		else if(Number(val2S) == -2){
			num21 = 300;
		}
		else if(Number(val2S) == -1){
			num21 = 275;
		}
		else if(Number(val2S) == 0){
			num21 = 250;
		}
		else if(Number(val2S) == 1){
			num21 = 225;
...
PrompTForBraces = (removeIstAndLastIndexAnswer,InputTextBox)=>{	
		...
		if(trimzero(removeIstAndLastIndexAnswer)==trimzero(String(InputTextBox).split("(").join("").split(")").join("")) && (InputTextBox.indexOf("(")==-1 || InputTextBox.indexOf(")")==-1)){
			...
			$(".blank").html("Please Enter");
			$(".blank1").html(" the Parentheses.");
			Prompt = true;
			...
			$(".blank").html("Please Check");
			$(".blank1").html(" Your Entry.");
			Prompt = true;
...
get_area = (num1x,num1y,num2x,num2y,num3x,num3y)=>{
    area = 0.5*(num1x * (num2y - num3y) + num2x * (num3y - num1y) + num3x* (num1y - num2y))
    return (area)
}
