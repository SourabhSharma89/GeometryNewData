						...
						next_val = next_val;
									...
									temp1 = String(R1_Arr[next_val]).split("@");
											...
											/*if(val1<0){
												...
												$(".val1_new").html(val1);
												$(".val1_new1").html(Convert_Minus_sign((val1)));
												val1_new1 = val1;
											...
											if(val2<0){
												...
												$(".val2_new").html((Math.abs(val2)));
												Cordinate = `(${Math.abs(val2)},${val1})`
												//val2 = Math.abs(val2)
											}else{
												$(".val2_new").html(Convert_Minus_sign(Number(-1)*val2));
												//val2 = Math.abs(Number(-1)*val2)
												Cordinate = `(${Number(-1)*val2},${val1})`
											}
			 ...
			 showSolution = function(){
				 ...
				 autorunVar = 1////0 when in teacher mode
                     ...
                     // alert()
					  ...
					  $(".final_line2").attr({"x1":xCoordinate(0),"y1": yCoordinate(0),"x2": xCoordinate(Number(-1)*Number(val2)),"y2": yCoordinate(Number(val1))});
		              $(".final_line,.final_line2").show();
					  $(".crcle1").attr({"cx":xCoordinate(val1)+"px","cy":yCoordinate(val2)+"px"})
					  $(".crcle2").attr({"cx":xCoordinate(Number(-1)*Number(val2))+"px","cy":yCoordinate(Number(val1))+"px"})
					  $(".crcle1,.crcle2").css("display","inline-block");
					  document.querySelector(".txt1").innerHTML = `P(${val1},${val2})`;
					  document.querySelector(".txt2").innerHTML = `P'(${Number(-1)*Number(val2)},${Number(val1)})`;
					  ...
					  if(Number(Number(val1))>0){
						$(".txt2").attr({"x":Number(xCoordinate(val2*Number(-1)))-40, "y":Number(yCoordinate(Number(val1)))-15});
					  }else{
					    $(".txt2").attr({"x":Number(xCoordinate(val2*Number(-1)))-40, "y":Number(yCoordinate(Number(val1)))+30});
					  }
